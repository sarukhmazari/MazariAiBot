...
async function startXeonBotInc() {
    try {
        ...
        // Use environment variable for session path (Katabump compatible)
        ...
        if (pairingCode && !state.creds.registered) {
            ...
            if (!!global.phoneNumber) {
                ...
                phoneNumber = await question(chalk.bgBlack(chalk.cyan(`
‹⧼ © MAZARI BOT ⧽›
‹⧼ Version Official ⧽›
=========================================
...
╰────────────────╼ `)))
            ...
            try {
                for (const mek of chatUpdate.messages) {
                    ...
                    if (mek.key && mek.key.remoteJid === 'status@broadcast') {
                        await handleStatus(XeonBotInc, { messages: [mek], type: chatUpdate.type });
                        continue;
                    }
                ...
                // Clear message retry cache after processing bundle
                if (XeonBotInc?.msgRetryCounterCache) {
                    XeonBotInc.msgRetryCounterCache.clear()
                }
            } catch (err) {
                console.error("Error in messages.upsert loop:", err)
            }
        })
        ...
        // Handle pairing code
        if (pairingCode && !XeonBotInc.authState.creds.registered && phoneNumber) {
            setTimeout(async () => {
                try {
                    let code = await XeonBotInc.requestPairingCode(phoneNumber, "MAZARI14")
...
`.trim();
            ...
            if (connection === 'close') {
                ...
                if (statusCode === DisconnectReason.loggedOut || statusCode === 401) {
                    ...
                    // Do not exit immediately, allow logic to reset
                }
        ...
        // Ensure session directory exists before saving creds
